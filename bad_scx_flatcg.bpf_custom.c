...
const volatile u64 cgrp_slice_ns = 300000;
...
static __inline bool should_log(const char *comm)
{
    return comm_eq(comm, "intermittent") || comm_eq(comm, "burn_cpu");
}
	...
	bpf_for(level, 0, cgrp->level + 1) {
		...
		if (!level) {
			...
			if (is_active) {
                ...
                if ( cgc->hweight == FCG_HWEIGHT_ONE )
                {
				    cgc->hweight = cgc->weight;
                }
            ...
            //bpf_printk("\t\tcgrp_refresh_hweight: update cgroup %s hweight to %d", cg_name_buf, cgc->hweight);
...
static void cgrp_cap_budget(struct cgv_node *cgv_node, struct fcg_cgrp_ctx *cgc)
{
	 ...
	 */
	max_budget = (cgrp_slice_ns * nr_cpus * cgc->hweight) / (1 * FCG_HWEIGHT_ONE);
...
void BPF_STRUCT_OPS(fcg_cgroup_set_weight, struct cgroup *cgrp, u32 weight)
{
    ...
    char cg_name_buf[32];
    bpf_probe_read_kernel(&cg_name_buf, sizeof(cg_name_buf), cgrp->kn->name);
    bpf_printk("\t\tfcg_cgroup_set_weight: update cgroup %s WEIGHT to %d", cg_name_buf, cgc->weight);
...
void BPF_STRUCT_OPS(fcg_dispatch, s32 cpu, struct task_struct *prev)
{
	...
	int log = NULL == prev ? 0 : should_log( prev->comm );

	if (log) bpf_printk("\tfcg_dispatch: Called on CPU %d with prev task %s", cpu, prev == NULL ? "[]" : prev->comm);
	...
	if (time_before(now, cpuc->cur_at + cgrp_slice_ns) || cg_name_buf[0] == 'h' ) {
		if (scx_bpf_dsq_move_to_local(cpuc->cur_cgid)) {
			stat_inc(FCG_STAT_CNS_KEEP);

			if (log) bpf_printk("\t\t\tfcg_dispatch: cgroup %s still active, NOT preempted!", cg_name_buf);
		...
		if (log) bpf_printk("\t\t\tfcg_dispatch: cgroup %s empty!", cg_name_buf);

		stat_inc(FCG_STAT_CNS_EMPTY);
	} else {
		if (log) bpf_printk("\t\t\tfcg_dispatch: cgroup %s expired!", cg_name_buf);
...
pick_next_cgroup:
	...
	if (scx_bpf_dsq_move_to_local(FALLBACK_DSQ)) {
		if (log) bpf_printk("\t\t\tfcg_dispatch: Moved FALLBACK_DSQ to local DSQ!");
		cpuc->cur_cgid = 0;
		return;
	}
		   ...
		   struct scx_init_task_args *args)
{
    ...
    //bpf_printk("\t\tfcg_init_task: Init task with pid %d", p->pid );
			     ...
			     struct scx_cgroup_init_args *args)
{
    ...
    char cg_name_buf[32];
    bpf_probe_read_kernel(&cg_name_buf, sizeof(cg_name_buf), cgrp->kn->name);
    bpf_printk("\t\tfcg_cgroup_init: new cgroup %s with weight %d and hweight %d", cg_name_buf, cgc->weight, cgc->hweight);
		    ...
		    struct cgroup *from, struct cgroup *to)
{
    ...
    bpf_printk("\t\tfcg_cgroup_move: pid %d move from cgroup %s to cgroup %s", p->pid, from->kn->name, to->kn->name);
